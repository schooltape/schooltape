...
function getPeriodData(periodNum) {
  if (periodNum !== undefined) {
    ...
    try {
      periodData.topPeriodTime = document
        .querySelector(`.timetable thead tr th:nth-child(${periodNum}) time`)
        .textContent.trim();
      periodData.topPeriodName = document
        .querySelector(`.timetable thead tr th:nth-child(${periodNum})`)
