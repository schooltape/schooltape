  ...
  defineStPlugin(
    ...
    () => {
      if (window.location.pathname === "/" && document.querySelector(".timetable")) {
        ...
        const timetableContainer = document.querySelector("[data-timetable-container]");
        if (timetableContainer) {
          timetableContainer.classList.add("columns");
          // get the row element and move the timetable container to the beginning
          const rowElement = document.querySelector(".Component_Dashboard_TimetableController")?.parentNode?.parentNode;
          if (rowElement) {
