...
<footer class="mt-4 flex min-w-full justify-around p-4">
  <span class="relative inline-flex">
    <Button
      onclick={async () => {
        await updated.set({ changelog: false });
      ...
      {#if updated.state.changelog}
        <span class="absolute right-0 top-0 -mr-1 -mt-1 flex size-3">
          <span class="bg-ctp-blue absolute inline-flex h-full w-full animate-ping rounded-full opacity-75"></span>
          <span class="bg-ctp-blue relative inline-flex size-3 rounded-full"></span>
        </span>
      {/if}
    </Button>
  </span>
  <div class="flex gap-3">
    ...
    <Button
      ...
      ><svg class="fill-ctp-text h-[22px]" role="img" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"
